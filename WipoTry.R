...
AllOther<-AllOther%>%select(RegNo,Task,REGRDAT,NOTDATE,REGEDAT,PUBDATE,INTOFF,DESPG2,DESPG,DESAG,DCPCD)
...
vlookup<-unique(forAllOthers[forAllOthers$Parent_Child=="Parent",c(1,32,6)])

AllOther<-unique(left_join(AllOther,vlookup, by=c("RegNo"="International Registration Number")))
...
AllOther <- AllOther %>%
  mutate(Designations = (paste(
    ...
    ifelse(
      is.na(`Designated Contracting Party Code`),
      "",
      `Designated Contracting Party Code`
    ),
    sep = ","
  ))) %>%
  select(RegNo,TRADEMARK,Task,REGRDAT,NOTDATE,REGRDAT,PUBDATE,INTOFF, Designations) %>%
  mutate(Designations =gsub(',+', ',', gsub('^,+|,+$', '', Designations)))%>%
  mutate(Designations = (strsplit(as.character(Designations), ","))) %>%
  unnest(Designations)
...
write_xlsx(final, path = "WIPOPowerPivot.xlsx")
